...
printwriter.println("HE;"+mUsername+";"+mPasswort+";"+s+";"+s5+";"+s4+";");            
    ...
    public String Registrieren(String s, String s1)
    {
        ...
        try
        {
            ...
            String s5 = Util.DownloadURL("http://testsys.twserver.org/pattern.php?Username="+mUsername+"&Passwort="+mPasswort);
            String s6 = "";
            for(int i = 1; i <= s5.length() - 1; i++)
                s6 = s6+Character.toString(s5.charAt(i));

            s5 = s6;
            s3 = s3+mUsername+";"+mPasswort+";"+s+";"+s1+";";
            printwriter.println(s3);
            byte abyte0[] = Util.XorPattern(Util.StrToByteArr(mPasswort), Util.StrToByteArr(s5));
            try
            {
                try
                {
